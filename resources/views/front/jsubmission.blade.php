  ...
  <div class="main-layout" data-v-08abc850="">
    ...
    <div class="main-container" data-v-08abc850="">
      ...
      <div class="dark-page">
        ...
        <!-- Product Card -->
        <div class="jsub-product-card">
          <div class="jsub-product-img">
            <img src="/backend/productImage/<?php echo $rewards->productImage; ?>" alt="<?php echo $rewards->productName; ?>">
          </div>
          <div class="jsub-product-info">
            <h3 class="jsub-product-name"><?php echo $rewards->productName; ?></h3>

            <!-- Star Rating -->
            <div class="jsub-rating-wrap">
              <div class="jsub-stars" id="starRating">
                <span class="jsub-star filled" data-value="1">&#9733;</span>
                <span class="jsub-star filled" data-value="2">&#9733;</span>
                <span class="jsub-star filled" data-value="3">&#9733;</span>
                <span class="jsub-star filled" data-value="4">&#9733;</span>
                <span class="jsub-star" data-value="5">&#9733;</span>
              </div>
              <span class="jsub-rating-text" id="ratingText">4.0 / 5.0</span>
            </div>
          </div>
        </div>

        <!-- Submit Button -->
        <button type="button" data-user-id="<?php echo $pendingProduts->id; ?>" class="submit dark-submit-btn" style="margin-top: 16px;">
...
<!-- Loading Overlay -->
<div class="jsub-overlay" id="overlay">
  <div class="jsub-loader"></div>
</div>
...
<style>
  ...
  .jsub-star:hover { transform: scale(1.15); }
  .jsub-rating-text {
    font-size: 13px;
  ...
  .jsub-loader {
    ...
    width: 44px;
    height: 44px;
    animation: jsubSpin 0.8s linear infinite;
    position: absolute;
    left: 50%;
    top: 50%;
  ...
  @media (max-width: 500px) { .swal2-popup { width: 90% !important; } }
  @media (min-width: 501px) { .swal2-popup { width: 400px !important; } }

  @media (max-width: 400px) {
    .jsub-product-name { font-size: 15px; }
    .jsub-star { font-size: 18px; }
    .jsub-rating-text { font-size: 11px; }
  }
</style>
...
<script>
  $(document).ready(function() {
    // Randomize initial rating
    var ratings = [3.5, 4.0, 4.0, 4.5, 4.5, 4.5, 5.0];
    var randomRating = ratings[Math.floor(Math.random() * ratings.length)];
    var fullStars = Math.floor(randomRating);
    $('#starRating .jsub-star').each(function(index) {
      if (index < fullStars) { $(this).addClass('filled'); }
      else { $(this).removeClass('filled'); }
    });
    $('#ratingText').text(randomRating.toFixed(1) + ' / 5.0');

    // Interactive star rating
    $('#starRating .jsub-star').on('click', function() {
      var value = $(this).data('value');
      $('#starRating .jsub-star').each(function(index) {
        if (index < value) { $(this).addClass('filled'); }
        else { $(this).removeClass('filled'); }
      });
      $('#ratingText').text(value + '.0 / 5.0');
    });

    // Hover effect
    $('#starRating .jsub-star').on('mouseenter', function() {
      var value = $(this).data('value');
      $('#starRating .jsub-star').each(function(index) {
        if (index < value) { $(this).addClass('hover'); }
        else { $(this).removeClass('hover'); }
      });
    });
    $('#starRating').on('mouseleave', function() {
      $('#starRating .jsub-star').removeClass('hover');
    });
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            if (response == 'success') {
              ...
              setTimeout(function() { window.location.href = "journey"; }, 4000);
            }
            if (response == 'error') {
              Swal.fire({
                icon: 'error',
